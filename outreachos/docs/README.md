# OutreachOS — Phase documentation

Step-by-step setup and **manual test checklists** for each phase.

| Doc | Phase | Status |
|-----|-------|--------|
| [PHASE1_SCAFFOLD.md](./PHASE1_SCAFFOLD.md) | Scaffold + Supabase | ✅ |
| [PHASE2_SUPABASE_AUTH.md](./PHASE2_SUPABASE_AUTH.md) | Auth + session | ✅ |
| [PHASE3_DATABASE.md](./PHASE3_DATABASE.md) | Database SQL | ✅ |
| [PHASE4_UI_SHELL.md](./PHASE4_UI_SHELL.md) | UI shell + components | ✅ |
| [PHASE5_BUSINESSES.md](./PHASE5_BUSINESSES.md) | Businesses CRUD | ✅ |
| [PHASE5_REALTIME.md](./PHASE5_REALTIME.md) | Realtime (optional) | ✅ |
| [PHASE6_DECISION_MAKERS.md](./PHASE6_DECISION_MAKERS.md) | Decision makers | ✅ Complete |
| [PHASE7_ACTIVITIES.md](./PHASE7_ACTIVITIES.md) | Activities | ✅ Complete |
| [PHASE8_DASHBOARD.md](./PHASE8_DASHBOARD.md) | Dashboard Kanban | ✅ Complete |
| [PHASE9_REMINDERS.md](./PHASE9_REMINDERS.md) | Notifications | ✅ Complete |
| [PHASE10_SETTINGS.md](./PHASE10_SETTINGS.md) | Templates + settings | ✅ Complete |
| [PHASE11_DISTRIBUTION.md](./PHASE11_DISTRIBUTION.md) | Installer + README | ⬜ Not started |

**Also read:** [../PHASES.md](../PHASES.md) (master plan) · [../PROGRESS.md](../PROGRESS.md) (live status)
